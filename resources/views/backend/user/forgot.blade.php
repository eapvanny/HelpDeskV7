...
@section('pageContent')
    ...
    <div class="login-box">
        ...
        <div class="login-box-body">
            <p class="login-box-msg text-danger"> {{ __('Use your email to get reset password link') }} </p>
            <form novalidate id="forgotForm" action="{{URL::Route('forgot')}}" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <!-- /.col -->
                    <div class="col-xs-6">
                        <a href="{{URL::Route('login')}}" class="login-link"> {{ __('Let me login') }} </a>
                    </div>
                    <!-- /.col -->
                </div>
                <br>
                <button type="submit" class="btn btn-lg btn-block btn-flat login-button"> {{ __('SEND RESET LINK') }} </button>
            </form>
...
<style>
    .hold-transition{
        background: url('../../../images/login-bg-2.jpg') !important;
