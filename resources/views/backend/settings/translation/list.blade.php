...
@section('pageContent')
    ...
    <section class="content">
            ...
            <div class="col-md-12">
                ...
                <div class="wrap-outter-header-title">
                    ...
                    <div class="box-tools pull-right">
                        ...
                        <button type="button" class="btn btn-primary btn-md" id="btnAddNew" data-bs-toggle="modal"
                            data-bs-target="#editTranslateModal"><i class="fa-solid fa-plus"></i>
                            {{ __('Add New') }}</button>
                    </div>
                </div>
                    ...
                    <div class="box-info">
                                                ...
                                                <div class="row">
                                                    <div class="col-12 col-md-6 col-xxl-3">
                                                        <label class="form-label"
                                                            for="search_locale">{{ __('File Translate As') }}:</label>
                                                        <select
                                                            class="form-select select2 border-0 form-select-custom fs-6 text-secondary"
                                                            aria-label="localization" id="search_locale" name="locale">
                                                            @foreach ($locales as $key)
                                                                <option {{ $key == $locale ? 'selected' : '' }}
                                                                    value="{{ $key }}">
                                                                    {{ $key == 'en' ? __('English') : ($key == 'kh' ? __('Khmer') : '') }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                        ...
                        <div class="row my-2">
                            <div class="col-12">
                                ...
                                <a class="btn btn-primary btn-sm" href="{{ route('translation.export') }}"><i
                                        class="fa-solid fa-download"></i> {{ __('Export') }}</a>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table id="listDataTableWithSearch"
                                class="table table-bordered table-striped display responsive no-wrap" width="100%">
                                <thead>
                                    <tr>
                                        <th>{{ __('Text') }}</th>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="change-action-item">
                                                    <div class="btn-group">
                                                        <a title="Edit" href="#" data-item="{{ $trans->item }}"
                                                            data-text="{{ $trans->text }}"
                                                            class="btn btn-primary btn-sm btn-edit-translate"
                                                            data-bs-toggle="modal" data-bs-target="#editTranslateModal"><i
                                                                class="fa fa-edit"></i></a>
                                                    </div>
                                                    <!-- todo: have problem in mobile device -->
                                                    <div class="btn-group">
                                                        <form class="myAction" method="POST"
                                                            action="{{ URL::route('translation.destroy', $trans->id) }}">
                                                            @csrf
        ...
        <div class="modal fade" id="editTranslateModal" tabindex="-1" aria-labelledby="editTranslateModalLabel"
            aria-hidden="true">
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="form-group has-feedback item-container">
                                        ...
                                        <input type="text" name="item" class="form-control" id="item"
                                            placeholder="classroom" value="" minlength="1" maxlength="255"
                                            required />
                                        <span class="fa fa-info form-control-feedback"></span>
                                        <span class="text-danger">{{ $errors->first('item') }}</span>
                                    </div>
                                    <div class="form-group has-feedback">
                                        <label for="text">{{ __('Translate As') }} <span
                                                class="text-danger">*</span></label>
                                        <textarea name="text" class="form-control" id="text" placeholder="ថ្នាក់រៀន" value="" rows="3"
                                            required></textarea>
                                        <span class="fa fa-info form-control-feedback"></span>
        ...
        <x-backend.import-modal modalId="importModal" formAction="{{ route('translation.import') }}"
            sampleFileUrl="{{ asset('example/translation-import-sample.xlsx') }}">
            <div class="mb-3">
    ...
    <script type="text/javascript">
        ...
        $("#apply_filter").click(function() {
            ...
            $(".wrap_filter_form .select2").each(function() {
                if ($(this).val() > 0) {
                    $("#filterForm").append("<input type='hidden' value='" + $(this).val() + "' name='" + $(
                        this).attr('name') + "'>");
                    i++;
                }
            });
